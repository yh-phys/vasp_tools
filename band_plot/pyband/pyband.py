...
def bandplot(kpath, bands, efermi, kpt_bounds, opts, whts=None):
    ...
    for bd in kpt_bounds:
        ax.axvline(x=bd, ls='--', color='k', lw=0.5, alpha=1)

    ax.set_ylabel('Energy (eV)', fontname='Times New Roman', 
            #labelpad=3,
            fontsize=15
            )
    ax.axhline(y=0, xmin=0, xmax=1, linestyle='--', linewidth=0.5, color='k')
    ...
    if opts.kpts:
        ...
        ax.set_xticklabels(kname, fontname='Times New Roman', fontsize=13)
    else:
        ax.set_xticklabels([])
